...
void Data::read(){
	...
	inTSP >> dimension; // valor depois do string "DIMENSION:" � a dimens�o do problema
	...
	// Alocar matriz 2D
    distMatrix.assign(dimension, vector<double>(dimension, 0.0));
	...
	if ( typeProblem == "EXPLICIT" ) {
		...
		for ( int i = 0; i < dimension; i++ ) {
			for ( int j = 0; j < dimension; j++ ) {
				distMatrix[i][j] = floor ( CalcDistEuc ( xCoord.data(), yCoord.data(), i, j ) + 0.5 );
		...
		for ( int i = 0; i < dimension; i++ ) {
			for ( int j = 0; j < dimension; j++ ) {
				distMatrix[i][j] = ceil ( CalcDistEuc ( xCoord.data(), yCoord.data(), i, j ) );
		...
		while ( file.compare("NODE_COORD_SECTION") != 0 ) { //enquanto nao achar o string "NODE_COORD_SECTION" fa�a
			inTSP >> file;
		}
		// ler coordenadas
		...
		CalcLatLong ( xCoord.data(), yCoord.data(), dimension, latitude, longitude );
		...
		for ( int i = 0; i < dimension; i++ ) {
			for ( int j = 0; j < dimension; j++ ) {
				distMatrix[i][j] = CalcDistAtt ( xCoord.data(), yCoord.data(), i, j );
